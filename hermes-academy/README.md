...
The implementation is merged and validated through the Phase 10–13 quality gate, but the controlled persistent-capability preflight is still in progress. Treat this as **pre-release/experimental**, not yet production-validated. The current preflight uses the generic `goal_manage` bridge and profile-distribution `preload_skills` support merged in downstream Hermes PRs #24 and #26. Agency profiles declare `academy-continuing-education` as a preload so the learner contract is active before the first model turn. Final stock-install readiness requires equivalent support in the normal Hermes release. See [`../docs/CONTINUING_EDUCATION.md`](../docs/CONTINUING_EDUCATION.md) for current behavior and status.
