...
5. Every Academy faculty distribution preloads `teach-profile`. The instructor treats the newest learner message as authoritative. A submitted baseline enters **baseline diagnostic mode**, which starts from what the learner already demonstrated, teaches only the remaining gaps, and forbids a broad subject lecture or replay of old Bot Chat lessons. A submitted transfer enters explicit assessment mode. Assessment returns `MASTERED`, `NEEDS_CORRECTION`, or `BLOCKED`.
